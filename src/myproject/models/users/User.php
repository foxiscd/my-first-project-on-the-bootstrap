...
class User extends ActiveRecordEntity
{
    ...
    private function setPassword ($passwordHash)
    {
    ...
    public function nicknameUpdate (string $fields)
    {
        $this->setNickname($fields);
        $this->save();
    }

    public function passwordUpdate ($fields)
    {
        if (!$this->passwordVerify($fields['password'])) {
            throw new InvalidArgumentException('Не верный пароль');
        }
        if ($fields['password'] == $fields['newPassword']) {
            throw new InvalidArgumentException('пароль не изменился');
        }

        $this->setPassword($this->passwordHash($fields['newPassword']));
        $this->save();
        header('Location: /account/' . $this->getId());
    }
